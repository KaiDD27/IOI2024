  ...
  for (int i = 0; i < n; i++)     // n行
    for (int j = 0; j < m; j++) { // m列
      ...
      for (int iMask = 0; iMask < (1 << m); iMask++) { // iMask：
        ...
        if (iMask & 1 << (m - 1)) {
          f[now][(iMask << 1) & (~(1 << m))] += f[old][iMask];
          f[now][(iMask << 1) & (~(1 << m))] %= MOD;
        }
        // 如果当前格竖放，且第 m-1位必须为 0,且i不为第 0 行
        if (i && !(iMask & 1 << (m - 1))) {
          f[now][(iMask << 1) | 1] += f[old][iMask];
          f[now][(iMask << 1) | 1] %= MOD;
        }
        // 如果当前格横放，则要求j不是第0列，且iMask的第 0 位必须为
        // 0，且第 n-1位必须为 1
        if (j && (!(iMask & 1)) && (iMask & 1 << (m - 1))) {
          f[now][((iMask << 1) | 3) & (~(1 << m))] += f[old][iMask];
          f[now][((iMask << 1) | 3) & (~(1 << m))] %= MOD;
          // iMask末尾置为11，且原来的m-1移到了第m位，置0
        }
      }
    }
  cout << f[now][(1 << m) - 1] << endl;
  return 0;
}
